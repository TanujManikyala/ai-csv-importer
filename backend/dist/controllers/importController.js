...
exports.importBatchHandler = importBatchHandler;
exports.importFileHandler = importFileHandler;
const aiService_js_1 = require("../services/aiService.js");
const csvParser_js_1 = require("../utils/csvParser.js");
...
function processAndValidateBatch(rawRecords, mappedRecords) {
    const imported = [];
    const skipped = [];
    for (let i = 0; i < rawRecords.length; i++) {
        ...
        let status = (mapped.crm_status || 'DID_NOT_CONNECT');
        if (!ALLOWED_STATUSES.has(status)) {
            status = 'DID_NOT_CONNECT';
        }
        // Validate and sanitize data source
        let source = (mapped.data_source || '');
...
async function importBatchHandler(req, res) {
    try {
        ...
        const mapped = await (0, aiService_js_1.mapBatchToCRM)(records);
...
async function importFileHandler(req, res) {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
        ...
        const rawRecords = (0, csvParser_js_1.parseCSVBuffer)(req.file.buffer);
        ...
        for (let i = 0; i < rawRecords.length; i += BATCH_SIZE) {
            ...
            try {
                const mapped = await (0, aiService_js_1.mapBatchToCRM)(batch);
